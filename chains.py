...
async def extract_vision_data(input: StructureVisualExtractionInput):
    ...
    message = HumanMessage(
        content=[
            ...
            "- If the image is a LECTURE PAGE (explanatory text, theory, or examples, but NOT a list of questions for the student to answer), you MUST return an empty list.\n"
            "- If the image is an EXERCISE PAGE (contains a list of questions, problems, or exercises for the student to solve, regardless of formatting), proceed to extract all questions as described below.\n"
            "\n"
            "**How to decide:**\n"
            "- If the majority of the text is instructional, theoretical, or provides explanations/examples, and there are only a few questions embedded as examples, it is a LECTURE PAGE.\n"
